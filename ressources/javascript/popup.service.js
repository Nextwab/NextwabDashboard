...
Dashboard.service("PopupService", function($compile , $location){
	...
	vm_Popup.openNew = function( Config ){
		angular.element(document.body).append($compile('<popup-custom ng-style=\''+JSON.stringify(Config.Styles)+'\' class="'+Config.ClassName+'" config=\''+JSON.stringify(Config)+'\'></popup-custom>')(vm_Popup.dashboard.scope));
        $('.background_overlay').fadeIn();
	};

    // Close
	vm_Popup.close = function(Popup) {
        $('popup-custom').fadeOut().remove(); 
        $('.background_overlay').fadeOut();
    };
});

Dashboard.directive("popupCustom", function(NavigationService){
